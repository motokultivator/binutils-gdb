  ...
  while (1)
    {
      gold_debug(DEBUG_TARGET, "%d pass: %s", pass,
                 (this->state_ == RELAX ? "Relaxations" : (this->state_ == EXPAND ? "Expansions" : "Trampolines")));
                  ...
                  if (t.trampoline == -1ull && (address - 1024 >= first))
                  ...
                  else
                    {
                      start_new_area = t.trampoline != -1ull
                                       && (address - 1024 > balc_trampolines_[t.trampoline].address);
                    }
                }
            ...
            for (auto t : targets)
              for (size_t i = t.first; i <= t.last; i++)
                if (t.target == balc_trampolines_[i].target
                  ...
                  balc_trampolines_[i].target = balc_trampolines_[t.trampoline].address
